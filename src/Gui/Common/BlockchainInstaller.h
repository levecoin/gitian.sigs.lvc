...
private:
  ...
  const QDir m_levecoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfLevecoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _levecoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo levecoinBlockchainInfo() const;
